...
const BASE_URL =
  ...
  "https://zero-waste-production.up.railway.app";
...
function OrderTrackingStrip({ order, onDismiss }) {
  ...
  const [reviewDismissed, setReviewDismissed] = useState(() => {
    return (
      localStorage.getItem(
        "review_dismissed_" + (order.orderNumber || order.reservationId),
      ) === "true"
    );
  });
  const [reviewSubmitted, setReviewSubmitted] = useState(() => {
    return (
      localStorage.getItem(
        "review_submitted_" + (order.orderNumber || order.reservationId),
      ) === "true"
    );
  });
  ...
  useEffect(() => {
    const handleVendorUpdate = (e) => {
      ...
      const eventMatches =
        ...
        (e.detail.orderId &&
          String(e.detail.orderId) === String(order.orderNumber));

      if (eventMatches) {
        console.log(
          "✅ MATCH FOUND! Updating UI with status:",
          e.detail.newStatus,
        );
  ...
  useEffect(() => {
    const fetchOrderStatus = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const status =
            (data.data && data.data.order_status) ||
            (data.order && data.order.order_status) ||
            data.order_status;
      ...
      <div className="order-strip" style={{ borderColor: "#fca5a5" }}>
        <div className="order-strip-header">
          <div className="order-strip-meta">
            <div
              className="order-strip-icon-wrap"
              style={{ background: "#fef2f2", color: "#ef4444" }}
            >
              <X size={13} />
            </div>
            <span className="order-strip-number">
              #{order.orderNumber || order.reservationId}
            </span>
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <React.Fragment key={step.id}>
              <div
                className={
                  "ost-step " + (done ? "done" : active ? "active" : "pending")
                }
              >
          ...
          <button
            onClick={(e) => {
              ...
              const offerToAdd = {
                ...
                id: String(offer.id),
                discountedPrice:
                  offer.discount_price || offer.discountedPrice || 0,
                ...
                stock:
                  ...
                  999,
                quantity_available:
                  offer.quantity_available || offer.quantity || 999,
                image: imageUrl,
                vendor_id:
                  (offer.branch && offer.branch.vendor_id) ||
                  offer.vendor_id ||
                  null,
                branch_id:
                  offer.branch_id || (offer.branch && offer.branch.id) || null,
                vendor_name:
                  offer.vendor_name ||
                  (offer.branch &&
                    offer.branch.vendor &&
          ...
          if (offersData.length > 0) {
            const transformedOffers = offersData.map((offer, idx) => {
              ...
              return {
                ...
                rating:
                  source.average_rating !== undefined
                    ? source.average_rating
                    : source.rating || 0,
                category:
                  (source.branch &&
                    source.branch.vendor &&
                    source.branch.vendor.vendor_type) ||
                  (source.vendor && source.vendor.vendor_type) ||
                  "Others",
                vendor_id:
                  (source.branch && source.branch.vendor_id) ||
                  (source.vendor && source.vendor.id) ||
                  null,
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: "0.82rem", color: "#6b7280" }}>
                Based on your interests and previous orders
              </p>
            </div>
          </div>
          <div
                ...
                <div className="dropdown-menu">
                  {sortOptions.map((opt) => (
                    <div
                      ...
                      className={
                        "dropdown-item " +
                        (sortOption === opt.value ? "dropdown-item-active" : "")
                      }
                      onClick={() => {
                    ...
                    >
                      ...
                      <span
                        style={{
                          fontSize: "0.82rem",
                          fontWeight: 700,
                          color: currentRating > 0 ? "#92400e" : "#9ca3af",
                        }}
                      >
                        ...
                        onClick={(e) => {
                          ...
                          const offerToAdd = {
                            ...
                            discountedPrice:
                              ...
                              0,
                            originalPrice:
                              offer.originalPrice || offer.original_price || 0,
                          };
