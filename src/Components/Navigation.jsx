      ...
      (position) => {
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}`)
          ...
          .then((data) => {
            ...
            localStorage.setItem("userLocationLat", String(latitude));
            localStorage.setItem("userLocationLng", String(longitude));
            if (onSuccess) onSuccess(locationName);
          })
          .catch(() => {
            setSelectedLocation("Current Location");
            localStorage.setItem("userLocationName", "Current Location");
          });
      },
      (err) => {
          ...
          {!hideLocation && (
            <div
              style={{
                ...
                border: selectedLocation ? "1px solid #22c55e" : "1px solid #e5e7eb",
                background: selectedLocation ? "#f0fdf4" : "white",
              }}
              onClick={() => setShowMap(true)}
              onMouseEnter={(e) => { if (!selectedLocation) { e.currentTarget.style.background = "#f0fdf4"; e.currentTarget.style.borderColor = "#d1fae5"; } }}
              onMouseLeave={(e) => { if (!selectedLocation) { e.currentTarget.style.background = "white";   e.currentTarget.style.borderColor = "#e5e7eb"; } }}
            >
              <MapPin size={18} />
